...
func checkOOBLog4j(ctx context.Context, target *types.Target, endpoint *types.Endpoint, client scanner.HttpClient) []types.Finding {
	...
	log.Printf("[*] [OOB] DNS callback subdomain: %s", subdomain)
	log.Printf("[*] [OOB] Check manually at: https://dnslog.cn/?r=%s", subdomain)
	log.Printf("[*] [OOB] Target: %s | URL: %s", target.BaseURL, endpoint.URL)
